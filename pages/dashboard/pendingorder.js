...
const MyPendingOrder = ({ data }) => {
    ...
    return (<>
        <Meta title='My Order - Furniture' />
        <DashboardSidebar login={login}>
            <h1 className='text-center my-5 text-lg sm:text-2xl font-bold'>Pending Order</h1>
...
export const getServerSideProps = async (context) => {
    const { query } = context;
    const res = await fetch(`${backend}/order?id=${query.id}&status=pending`);
